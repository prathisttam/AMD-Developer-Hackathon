...
from rlm.agents import AgentLLMConfig, create_judge_agent, create_main_agent, create_sub_agent
from tools.agent_tools import repl_tool, set_active_sub_agent
...
class MainLoop:
    async def main_loop(
        self,
        query: str,
        main_judge_config: AgentLLMConfig,
        subagent_config: AgentLLMConfig,
    ) -> str:
        main_agent = create_main_agent(main_judge_config)
        judge_agent = create_judge_agent(main_judge_config)
        sub_agent = create_sub_agent(subagent_config)

        prompt = query
        best_response = ""
        best_score = 0.0

        set_active_sub_agent(sub_agent)
        try:
            for iteration in range(5):
                ...
                try:
                    ...
                    if inspect.isawaitable(judgement):
